...
bool checkNurseFeasibility(vector<float> Hours, int hour, int nurse, int totalHours, int maxHours, int maxConsecHours, int maxPresenceHours) 
{
	...
	if (accumulatedHours >= maxHours) return false; //{ cout << "hour: " << hour << " accumulatedHours: " << accumulatedHours << " maxHours: " << maxHours << endl; feasible = false; }
	
	/* The Nurse doesn't exceed the maxConsecHours */
	if (lastConsecHours >= maxConsecHours) return false;
	...
	if (firstWorkedHour != -1) {
		...
		if ((lastWorkedHour - firstWorkedHour) >= maxPresenceHours) return false;
	}
